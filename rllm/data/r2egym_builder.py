...
``/r2e_tests/``, and an expected-output JSON ``/expected_test_output.json``.
The agent fixes the bug; the verifier just runs ``/r2e_tests/run_tests.sh``
and compares the parsed pytest output against ``expected_output_json``.
...
# The verifier is dead simple compared to swebench_pro / swegym: the
# R2E-Gym image ships a self-contained ``/r2e_tests/run_tests.sh`` that
# runs the full test suite via pytest from inside the repo's bundled
# ``.venv``. We just invoke it (against whatever state the agent left
# /testbed in), parse the per-test PASSED/FAILED/ERROR statuses, and
# compare to the expected map. Reward = 1.0 iff EVERY expected test
# appears with the expected status — matching ``_calculate_reward_r2e``
# in ``r2egym.agenthub.runtime.docker``.
...
# R2E-Gym ships run_tests.sh at /r2e_tests/run_tests.sh in raw images.
# Some env-setup paths move it to /root/r2e_tests/ — probe both.
RUN_TESTS=""
for cand in /r2e_tests/run_tests.sh /root/r2e_tests/run_tests.sh; do
    if [ -f "$cand" ]; then
        RUN_TESTS="$cand"
        break
    fi
...
if [ -z "$RUN_TESTS" ]; then
    write_failure "run_tests.sh not found at /r2e_tests/ or /root/r2e_tests/"
    exit 0
fi

log "Running $RUN_TESTS"
chmod +x "$RUN_TESTS" 2>/dev/null || true
...
def _build_solution_script(has_patch: bool) -> str:
    ...
    Unlike SWE-Gym we don't ``git reset --hard`` first — the image is
...
def _load_rows(hf_repo_id: str, hf_split: str, *, retries: int = 4, backoff_sec: float = 10.0) -> list[dict]:
    ...
    Same hazard as :func:`rllm.data.swegym_builder._load_rows`: ``rllm eval``
    auto-pulls before running, so a transient ``LocalEntryNotFoundError``
    from a DNS/SSL blip would crash the eval. Cached partial progress
    makes the retries cheap.
    """
    import time
